...
class ConditionalRandomField(HiddenMarkovModel):
    ...
    def init_params(self) -> None:
        ...
        self.WA[:, self.bos_t] = float("-inf")  # no transitions to BOS
        if not self.unigram:
            self.WA[self.eos_t, :] = float("-inf")  # no transitions from EOS

        # no BOS/EOS emissions
    ...
    def updateAB(self) -> None:
        ...
        if self.unigram:
            Arow = torch.exp(self.WA)
            A = Arow.repeat(self.k, 1)          
        else:
            A = torch.exp(self.WA)             
    ...
    def logprob(self, sentence: Sentence, corpus: TaggedCorpus) -> TorchScalar:
        ...
        logZ_num = self.forward_pass(isent)
        logZ_den = self.forward_pass(desup_isent)
        return logZ_num - logZ_den
    ...
    def logprob_gradient_step(self, lr: float) -> None:
        ...
        if self.unigram:
            grad_WA = self.A_counts.sum(dim=0)
            self.WA[0, :] += lr * grad_WA
            self.WA[0, self.bos_t] = float("-inf")
        else:
