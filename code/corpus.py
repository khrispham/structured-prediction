...
class TaggedCorpus:
    ...
    def integerize_tag(self, tag: Tag) -> int:
        ...
        if t is None:
            raise KeyError(tag, self, f"This tag isn't in the tagset of this corpus (we don't support OOV tags)")
        return t
    ...
    def integerize_word(self, word: Word) -> int:
        ...
        if w is None: 
            ...
            if w is None:   # OOV_WORD needs to be in our tagset
                raise KeyError(word, self, f"This word isn't in the vocab of the corpus, and can't be converted to OOV since OOV is also missing from the vocab")
        return w
