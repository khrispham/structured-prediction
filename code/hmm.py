...
class HiddenMarkovModel:
    ...
    def M_step(self, λ: float) -> None:
        ...
        A_counts = self.A_counts.clone()
        A_counts[:, self.bos_t] = 0.0   
        A_counts[self.eos_t, :] = 0.0   

        if not self.unigram:
            # Add-λ ONLY to legal cells
            mask = torch.ones_like(A_counts, dtype=torch.bool)
            mask[:, self.bos_t] = False          # illegal targets
            mask[self.eos_t, :] = False          # illegal sources
            A_smoothed = A_counts + (λ * mask.float())

            row_sums = A_smoothed.sum(dim=1, keepdim=True) 
            safe_row_sums = torch.where(row_sums == 0, torch.ones_like(row_sums), row_sums)
            A_norm = A_smoothed / safe_row_sums
            A_norm[:, self.bos_t] = 0.0
            A_norm[self.eos_t, :] = 0.0
            self.A = A_norm
        else:
            uni = A_counts.sum(dim=0) + λ
            uni[self.bos_t] = 0.0
            total = uni.sum()
            if total == 0:
                uni = torch.ones_like(uni)
                uni[self.bos_t] = 0.0
                total = uni.sum()
            uni = uni / total
            A = uni.repeat(self.k, 1)
            A[:, self.bos_t] = 0.0
            A[self.eos_t, :] = 0.0
            rs = A.sum(dim=1, keepdim=True)
            rs = torch.where(rs == 0, torch.full_like(rs, 1.0), rs)
            self.A = A / rs
    ...
    def forward_pass(self, isent: IntegerizedSentence) -> TorchScalar:
        ...
        n = len(isent)  # BOS at 0, EOS at n-1
        logA = torch.log(self.A.clamp_min(1e-45))

        def obs_mask(tag_id: Optional[int]) -> Tensor:
            if tag_id is None:
                return torch.zeros(self.k)
            m = torch.full((self.k,), float("-inf"))
            m[tag_id] = 0.0
            return m

        def emit_log(j: int) -> Tensor:
            if j == 0 or j == n - 1:
                return torch.zeros(self.k)  # no emissions at BOS/EOS
            wj, _ = isent[j]
            return torch.log(self.B[:, wj].clamp_min(1e-45))

        a0 = torch.full((self.k,), float("-inf"))
        a0[self.bos_t] = 0.0
        alpha[0] = a0

        for j in range(1, n):
            prev = alpha[ j-1 ].unsqueeze(1) + logA
            aj = torch.logsumexp(prev, dim=0) 
            _, tj = isent[j]
            aj = aj + emit_log(j) + obs_mask(tj)
            alpha[j] = aj

        log_Z = alpha[-1][self.eos_t]

        self._alpha_log = alpha 
        self._logZ = log_Z      
        return log_Z
    ...
    def backward_pass(self, isent: IntegerizedSentence, mult: float = 1) -> TorchScalar:
        ...
        bN = torch.full((self.k,), float("-inf"))
        bN[self.eos_t] = 0.0
        beta[-1] = bN

        for j in range(n - 2, -1, -1):
            emit_next = emit_log(j + 1)
            _, t_obs = isent[j + 1]
            mask_next = obs_mask(t_obs)
            factors = emit_next + mask_next + beta[j + 1]
            temp = logA + factors.unsqueeze(0)
            beta[j] = torch.logsumexp(temp, dim=1)

        log_Z_backward = beta[0][self.bos_t]

        alpha_log = self._alpha_log
        log_Z = self._logZ          

        # emissions
        for j in range(1, n - 1):
            wj, _ = isent[j]
            gamma_log = alpha_log[j] + beta[j] - log_Z
            gamma = torch.exp(gamma_log).clamp_min(0.0)
            gamma[self.bos_t] = 0.0
            gamma[self.eos_t] = 0.0
            self.B_counts[:, wj] += mult * gamma
        ...
        for j in range(0, n - 1):
            ...
            mask_next = obs_mask(t_obs)
            xi_log = (
                alpha_log[j].unsqueeze(1)
                + logA
                + (emit_next + mask_next + beta[j + 1]).unsqueeze(0)
                - log_Z
            )
            xi = torch.exp(xi_log).clamp_min(0.0)
            xi[self.eos_t, :] = 0.0
            xi[:, self.bos_t] = 0.0
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        a0[self.bos_t] = 0.0
        alpha[0] = a0
        backpointers[0].fill_(-1)

        for j in range(1, n):
            prev = alpha[j - 1].unsqueeze(1) + logA 
            a_j, arg = torch.max(prev, dim=0) 
            _, tj = isent[j]
            a_j = a_j + emit_log(j) + obs_mask(tj)
            alpha[j] = a_j
            backpointers[j] = arg
        ...
        for j in range(n - 1, 0, -1):
            tags[j - 1] = int(backpointers[j][tags[j]].item())
